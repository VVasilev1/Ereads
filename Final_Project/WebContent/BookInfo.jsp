<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	...
	import="java.util.ArrayList" import="bg.ereads.classes.Review"%>
...
<body>
	...
	<div id="profileBox">
		<div style="display: table-row;">
			<div style="display: table-cell; width: 250px;">
				<a href="./DisplayPictureBook?picture=<%=book.getImage()%>"
					data-lightbox="<%=book.getImage()%>"> <img
					src="./DisplayPictureBook?picture=<%=book.getImage()%>"
					width="250" height="400"></a>
			</div>
			<div style="display: table-cell; width: 400px; vertical-align: top;">
				Title:<b><%=book.getName()%></b><br> Author:<b><%=book.getAutor()%></b><br>
				Genre:<b><%=book.getGenre()%></b><br> Buy from:<b><a
					href=<%=book.getLinkToBuy()%>>Here</a></b><br> Description:<b><%=book.getDescription()%></b><br>
				Rating:<b><%=dRating.format(book.getRating())%></b><br>
				<%
					if (request.getSession(false).getAttribute("user") != null) {
						out.println("Rate the book:<b>" + "<form name='rating' action='./RateBook' method='post'>"
								+ "Give rate:" + "<select name='rate' value='rate'>" + "<option value='1'>1</option>"
								+ "<option value='2'>2</option>" + "<option value='3'>3</option>"
								+ "<option value='4'>4</option>" + "<option value='5'>5</option>" + "</select>"
								+ "<input type='submit' value='Add rating'>" + "</form>" + "<form name='review' action='." + "/"
								+ "AddReview' method='post'>" + "<label>Comment</label><br/>"
	...
	<div id="Reviews">
 	...
 	for (Review review : reviews) {
 		out.println("<a href='./GetUser?image=" + review.getPicture() + "'><img src='./DisplayPicture?picture="
 				+ review.getPicture() + " 'width='50' height='50'>" + " " + review.getName() + " :"
